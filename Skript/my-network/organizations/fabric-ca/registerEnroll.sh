...
function createOrg1() {
  ...
  if [ -n "$1" ]; then
    ...
    errorln "Anzahl Peers wurde nicht an createOrg1 übergeben. Prozess wird abgebrochen"
    exit 1
  fi
  ...
  infoln "Registering peers (registerEnroll)"
  for ((i=0 ; i < ${PEER_COUNT}; i++)); do
  	infoln "Registering peer$i"
    echo test peer$ipw
    set -x
    fabric-ca-client register --caname ca-org1 --id.name peer$i --id.secret peer$ipw --id.type peer --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem"
    { set +x; } 2>/dev/null
  done  
  ...
  infoln "Generating the peers msp"
  for ((i=0 ; i < ${PEER_COUNT}; i++)); do
    infoln "Generating the peer$i msp"
    set -x
    fabric-ca-client enroll -u https://peer$i:peer$ipw@localhost:7054 --caname ca-org1 -M "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$i.org1.example.com/msp" --csr.hosts peer$i.org1.example.com --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem"
    { set +x; } 2>/dev/null
    ...
    set -x
    fabric-ca-client enroll -u https://peer$i:peer$ipw@localhost:7054 --caname ca-org1 -M "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$i.org1.example.com/tls" --enrollment.profile tls --csr.hosts peer$i.org1.example.com --csr.hosts localhost --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem"
    { set +x; } 2>/dev/null
