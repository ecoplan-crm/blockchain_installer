...
function checkCommitReadiness() {
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    ...
    #überprüft, ob die restlichen übergebenen parameter der funktion in der datei log.txt so vorhanden sind. wenn eines nicht vorhanden ist, dann wird rc=1 gesetzt und damit ein fehler markiert
    for var in "$@"; do
...
function commitChaincodeDefinition() {
  ...
  echo Hier folgt der nun fologende Shell-Befehl
  echo "peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile \"$ORDERER_CA\" --channelID $CHANNEL_NAME --name ${CC_NAME} ${PEER_CONN_PARMS[@]} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG}"
