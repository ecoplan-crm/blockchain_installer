...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $ORG_COUNT $PEER_COUNT
}

function setGradleRootProjectName() {

file_name=$CC_SRC_PATH"settings.gradle"
...
function deployCC() {
  ...
  #Generieren der settings.gradle
  setGradleRootProjectName

  scripts/deployCC.sh $ORG_COUNT $PEER_COUNT $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  COMPOSE_PEER_FILES="-f compose/compose-peerXorg1.yaml"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    #DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} down --volumes --remove-orphans
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_PEER_FILES} down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.org1.example.com docker_peer1.org1.example.com docker_peer2.org1.example.com
...
function editHosts() {
  ...
  infoln "Hosts-Datei wird bearbeitet."

  removeHostsEntry ".example.com"

  setHostsEntry "orderer.example.com"
  setHostsEntry "org1.example.com"

  for ((i=0 ; i < ${PEER_COUNT}; i++)); do
    setHostsEntry "peer$i.org1.example.com"
  done

  IP="127.0.0.1"
  setHostsEntry $MYDOMAIN

  infoln "Hosts-Datei wurde bearbeitet"
}

removeHostsEntry() {
    domain=$1
    hostsDatei="/etc/hosts"
    sudo cp "$hostsDatei" "$hostsDatei.bak"
    sudo grep -v "$domain" "$hostsDatei.bak" | sudo tee "$hostsDatei"
    sudo rm "$hostsDatei.bak"
    println "Zeilen mit '$domain' wurden aus hosts-Datei entfernt."
}

# Überschreib die hosts-Datei mir den richtigen werten
function setHostsEntry() {

  domain=$1

  infoln "Host-Eintrag $domain wird bearbeitet"

  # Suchen des bestehenden Eintrags
  existingEntry=$(grep -w "$domain" /etc/hosts)

  if [ -n "$existingEntry" ]; then
    # Eintrag existiert: Ersetze ihn
    sudo sed -i "s|$existingEntry|$IP $domain|" /etc/hosts
    println "Hosts-Eintrag '$existingEntry' war vorhanden und wurde durch '$IP $domain' ersetzt."
  else
    # Eintrag existiert nicht: Füge neuen Eintrag hinzu
    echo "$IP $domain" | sudo tee -a /etc/hosts >/dev/null
    println "Hosts-Eintrag '$IP $domain' war nicht vorhanden und wurde neu eingefügt."
  fi
}
...
if [ "$MODE" == "up" ]; then
  ...
  networkDown
elif [ "$MODE" == "editHosts" ]; then
  editHosts
elif [ "$MODE" == "setGradleRootProjectName" ]; then
  setGradleRootProjectName
